---
title: "自己紹介8"
image: "/yoshi/2.png"
author: "yoshi"
date: "2025-12-24"
---

## あいさつ

はじめまして、yoshi_pawn です。\
私は普段、理系の学生をしています。\
Nu.メタ創の新規メンバーでわからないことも多いですが、がんばります！

## 興味のあるツール

### Blender

昔からものづくりが好きで、高校の時に部活の先輩に教えてもらったのがきっかけです。
今では VRChat で使用するためのアバター制作を勉強中です。

<figure>
  <img src="/yoshi/1.png" alt="画像1" style="width:70%">
</figure>

<figure>
  <img src="/yoshi/2.png" alt="画像1" style="width:70%">
</figure>

<figure>
  <img src="/yoshi/3.png" alt="画像1" style="width:70%">
</figure>

<figure>
  <img src="/yoshi/4.png" alt="画像1" style="width:70%">
</figure>

### Unity

Blender つながりで、作った 3D モデルを用いたゲームを制作したいと思ったことがきっかけです。
勉強のために 2D でシューティングゲームやコマンドゲームを作っています。

<figure>
  <img src="/yoshi/5.png" alt="画像1" style="width:70%">
</figure>

<figure>
  <img src="/yoshi/6.png" alt="画像1" style="width:70%">
</figure>

### 目標

- ゲームキャラクターを Blender で制作する。
- 3D ゲームを Unity で制作する。
- ゲームを操作するためのコントローラーを電子工作する。
